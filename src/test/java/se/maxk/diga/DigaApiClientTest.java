...
class DigaApiClientTest {
  ...
  void testCodeParsingFailureRethrows() throws DigaCodeValidationException {
    var code = "wrong";
    Mockito.when(codeParser.parseCodeForValidation(code))
        .thenThrow(DigaCodeValidationException.class);
    Assertions.assertThrows(DigaCodeValidationException.class, () -> client.validateDigaCode(code));
  }

  @Test
  void testXmlWritingFailureRethrows() throws DigaXmlWriterException {
    Mockito.when(xmlRequestWriter.createCodeValidationRequest(Mockito.any()))
        .thenThrow(DigaXmlWriterException.class);
    Assertions.assertThrows(DigaXmlWriterException.class, () -> client.validateDigaCode("hi"));
  }

  @Test
  void testExceptionAfterXmlWritingReturnsResponseCodeValidation()
      throws DigaEncryptionException, DigaXmlWriterException, DigaCodeValidationException {
  ...
  @Test
  void testExceptionAfterXmlWritingReturnsResponseBilling()
      throws DigaEncryptionException, DigaXmlWriterException, DigaCodeValidationException {
    var invoice =
        DigaInvoice.builder().invoiceId("1").validatedDigaCode("code").digavEid("12345000").build();
  ...
  @Test
  void testNonApiEndpointsReturnManualActionResponse()
    ...
    Mockito.when(codeParser.parseCodeForBilling(Mockito.anyString())).thenReturn(info);
    Mockito.when(xmlRequestWriter.createBillingRequest(invoice, info))
        .thenReturn(generatedInvoice.getBytes(StandardCharsets.UTF_8));
